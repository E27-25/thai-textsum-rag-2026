"""
32B SFT v3: ดันเพดาน 0.68 → 0.72+
  - N_DISTRACTORS=10 (มากกว่า v1's 7 — train robustness against larger context)
  - Each query → 3 examples: original gt_abst + 2 paraphrases (data aug ×3)
  - Same chat template, same SYSTEM_PROMPT (matches v1 baseline + inference)
  - 3 epochs (effective ~3.7k × 3 = 11k gradient updates, similar to v1's 1239×4)
  - Output: lora_output_32b_v3 → Qwen3-32B-SFT-v3
"""
import os
...
PARA_PATH    = BASE / "paraphrases_v3.json"
OUTPUT_DIR   = str(BASE / "lora_output_32b_v3")
...
if PARA_PATH.exists():
    ...
    print(f"  {len(paraphrases)} entries")
else:
    print(f"  WARNING: {PARA_PATH} not found — training without augmentation")
    paraphrases = {}
...
def get_hard_negatives(query_text, doc_id, correct_ids, n=N_DISTRACTORS):
    ...
    available = [p for p in paras if p["para_id"] not in correct_ids]
    if len(available) < n:
        return available
...
for q in tqdm(queries, desc="  preparing"):
    ...
    # Collect abstract variants: original + paraphrases
    variants = [q["abstractive"]]
    for p in paraphrases.get(q["ID"], []):
        if p and len(p) > 10 and p != q["abstractive"]:
            variants.append(p)

    for abst_var in variants:
        # Re-shuffle distractors per variant so model sees different orderings
        negs_copy = list(hard_negs)
        random.shuffle(negs_copy)
        context = negs_copy + oracle   # oracle last
        prompt = build_prompt(q["query"], context)
        answer = json.dumps({"abstractive": abst_var, "refs": q["refs"]}, ensure_ascii=False)
        messages = [
...
val_size = max(50, int(len(examples) * 0.05))
