...
PODMAN_ROOT="/dev/shm/podman-root-refboost-${USER}"
PODMAN_RUN="/tmp/podman-run-refboost-${USER}"
...
echo "=== [$(date)] Build start (32B RefBoost) ==="
...
podman build \
  ...
  --ignorefile .dockerignore.32b_refboost_inference \
  -t textsum-32b-refboost:v1 \
  -f Dockerfile.32b_refboost \
  .
...
for attempt in 1 2 3 4 5; do
  ...
  podman push \
    --root="$PODMAN_ROOT" \
    --runroot="$PODMAN_RUN" \
    textsum-32b-refboost:v1 \
    'registry.ai.in.th/2026-textsum/48f0b4ab/watin-promfiy.tme5:AI-Benchmark-Programs-2026-32b-refboost-v2' && break
  echo "Push failed, retrying in 60s..."
  sleep 60
done

echo "=== [$(date)] Push done! ==="
echo "Tag: AI-Benchmark-Programs-2026-32b-refboost-v2"
