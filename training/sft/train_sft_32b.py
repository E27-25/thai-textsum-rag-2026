"""
QLoRA SFT for Qwen3-32B on Thai meeting-minutes dataset.
Same approach as SFT v5 but with 32B model for better style learning.
Key settings:
  - System prompt matches inference
  - Oracle-last paragraph ordering
  - 3 hard negatives (keeps context clean → better RougeL)
  - 3 epochs (32B learns faster)
  - Output: lora_output_32b → Qwen3-32B-SFT-v1
"""
...
OUTPUT_DIR  = str(BASE / "lora_output_32b")
...
for q in tqdm(queries, desc="  preparing"):
    ...
    context_paras = hard_negs + oracle_paras  # oracle last (most-relevant-last)
...
print("Loading 32B model (QLoRA 4-bit)...")
...
lora_cfg = LoraConfig(
    r=32,           # smaller rank for 32B (saves memory, still effective)
    lora_alpha=64,
