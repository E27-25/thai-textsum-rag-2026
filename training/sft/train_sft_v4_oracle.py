"""
QLoRA SFT v4 Oracle for Qwen3-14B.
Key change from v3:
  - Context = ground-truth ref paragraphs (oracle) + top-3 hard negatives
  - No retrieval noise → model learns better abstractive quality
  - Hard negatives: dense-retrieved non-refs (challenging distractors)
  - 3 epochs
"""
...
OUTPUT_DIR  = str(BASE / "lora_output_v4")
...
def build_prompt(query: str, context_paras: list[dict]) -> str:
    # Most relevant last (oracle refs shuffled, so just keep order)
...
print("Building training examples (oracle + hard negatives)...")
...
for q in tqdm(queries, desc="  preparing"):
    ...
    context_paras = oracle_paras + hard_negs
    random.shuffle(context_paras)
