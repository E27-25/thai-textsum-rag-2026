#!/usr/bin/env python3
"""Two-stage inference:
   Stage 1: model picks refs from retrieved paras (~80 tokens)
   Stage 2: model writes abstractive given ONLY selected paras (~150 tokens)
"""
...
# Tight budget — 2 LLM calls means we need to be fast
TOP_K_RETRIEVE = 15
TOP_K_FINAL    = 5
SPARSE_WEIGHT  = 0.3
BM25_WEIGHT    = 0.2
STAGE1_MAX_TOKENS = 80    # refs only
STAGE2_MAX_TOKENS = 130   # abstractive
GPU            = "cuda:0"
BGE_BATCH      = 128
...
def build_stage1_prompt(query, retrieved, max_para_chars=800):
    """Ask model to pick refs only (no abstractive yet)."""
    ...
    return (f"ย่อหน้าจากบันทึกการประชุม:\n{context}\n\n"
            f"คำถาม: {query}\n\nให้เลือก ID ย่อหน้าที่ตอบคำถามได้\n"
            f"ID ที่เลือกได้: {ids}\n\nตอบเป็น JSON เท่านั้น:\n"
            f'{{"refs": ["id1", "id2"]}}')


def build_stage2_prompt(query, oracle_paras, max_para_chars=1000):
    """Generate abstractive from ONLY the selected (oracle) paras — clean context."""
    context = "\n".join(f"[{p['para_id']}] {p['text'][:max_para_chars]}" for p in oracle_paras)
    return (f"ย่อหน้าจากบันทึกการประชุม:\n{context}\n\n"
            f"คำถาม: {query}\n\nให้สรุปคำตอบจากย่อหน้าข้างต้นเป็นภาษาไทยที่เป็นธรรมชาติ ครบถ้วน\n"
            f"ตอบเป็น JSON เท่านั้น:\n"
            f'{{"abstractive": "สรุปคำตอบภาษาไทย"}}')
...
def parse_abstractive(text, fallback):
    try:
        match = re.search(r'\{.*\}', text, re.DOTALL)
        if match:
            obj = json.loads(match.group())
...
def main():
    ...
    llm = LLM(model=LLM_PATH, dtype="auto", tensor_parallel_size=1,
              gpu_memory_utilization=0.92, max_model_len=8192, max_num_seqs=8,
              trust_remote_code=True)

    # ── STAGE 1: pick refs ────────────────────────────────────────────────
    print(f"[STAGE 1] Building ref-selection prompts ...", flush=True)
    stage1_prompts = []
    for q, retrieved in zip(queries, retrieved_all):
        messages = [{"role": "system", "content": SYSTEM_PROMPT},
                    {"role": "user", "content": build_stage1_prompt(q["query"], retrieved)}]
        stage1_prompts.append(tok.apply_chat_template(messages, tokenize=False,
            add_generation_prompt=True, enable_thinking=False))

    stage1_params = SamplingParams(temperature=0, max_tokens=STAGE1_MAX_TOKENS)
    print(f"[STAGE 1] Generating refs ...", flush=True)
    stage1_outputs = llm.generate(stage1_prompts, stage1_params)

    selected_paras_per_query = []
    selected_refs_per_query = []
    for q, retrieved, output in zip(queries, retrieved_all, stage1_outputs):
        fallback = retrieved[0]["para_id"] if retrieved else "P1"
        refs = parse_refs(output.outputs[0].text, fallback)
        # Map refs to para dicts
        ref_paras = [{"para_id": r, "text": para_text_map.get(r, "")} for r in refs if r in para_text_map]
        if not ref_paras:
            # fallback to top retrieved
            ref_paras = retrieved[:3]
            refs = [p["para_id"] for p in ref_paras]
        selected_paras_per_query.append(ref_paras)
        selected_refs_per_query.append(refs)

    # ── STAGE 2: write abstractive ────────────────────────────────────────
    print(f"[STAGE 2] Building abstractive prompts ...", flush=True)
    stage2_prompts = []
    for q, oracle_paras in zip(queries, selected_paras_per_query):
        messages = [{"role": "system", "content": SYSTEM_PROMPT},
                    {"role": "user", "content": build_stage2_prompt(q["query"], oracle_paras)}]
        stage2_prompts.append(tok.apply_chat_template(messages, tokenize=False,
            add_generation_prompt=True, enable_thinking=False))
    del tok

    stage2_params = SamplingParams(temperature=0, max_tokens=STAGE2_MAX_TOKENS)
    print(f"[STAGE 2] Generating abstractive ...", flush=True)
    stage2_outputs = llm.generate(stage2_prompts, stage2_params)

    # ── Verify refs with reranker ─────────────────────────────────────────
    ...
    rows = []
    for q, retrieved, refs, output in zip(queries, retrieved_all, selected_refs_per_query, stage2_outputs):
        fallback = retrieved[0]["para_id"] if retrieved else "P1"
        valid_ids = {p["para_id"] for p in doc_map[q["doc_id"]]["paragraphs"]}
        abstractive = parse_abstractive(output.outputs[0].text, "")
        refs = expand_contiguous(refs, valid_ids) if refs else [fallback]
