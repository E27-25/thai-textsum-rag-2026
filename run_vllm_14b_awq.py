...
TOP_K_RETRIEVE = 15   # v4: faster retrieval
TOP_K_FINAL    = 5    # v4: shorter prompt = faster gen
SPARSE_WEIGHT  = 0.3
BM25_WEIGHT    = 0.2
MAX_NEW_TOKENS = 120  # v4: median gt_abst ~175 chars (~60 tokens); 120 safe
GPU          = "cuda:0"
BGE_BATCH    = 128
...
def parse_output(text: str, fallback_para: str, valid_para_ids: set[str] | None = None) -> tuple[str, list[str]]:
    """Robust parser — handles truncated JSON (max_tokens cut closing brace).
    Strategy: try full JSON first, then fall back to regex field extraction."""
    ...
    # Strategy 2: truncated JSON — extract fields by regex even without closing brace
    abstractive = ""
    refs: list[str] = []
    # Extract "abstractive": "..."  (handles \n, \", etc.)
    ...
    # Extract refs list — find "refs": [ ... ] (handle missing closing bracket)
    ...
    # Strategy 3: pure plaintext — strip any leading {"...":"
...
def main():
    ...
    llm = LLM(
        ...
        max_model_len=6144,
        max_num_seqs=16,           # 14B uses ~8GB → more room for parallel sequences
    ...
    NEIGHBOR_THRESHOLD    = 0.20   # v4: lower to catch more neighbors (EDA: 95% under-predict refs)

    def _cross_score(query: str, para_ids: list[str]) -> list[float]:
        pairs = [[query, para_text_map[r][:500]] for r in para_ids]   # v4: truncate 800→500
        enc = reranker_tok(
            [p[0] for p in pairs], [p[1] for p in pairs],
            padding=True, truncation=True, max_length=384, return_tensors="pt"   # v4: 512→384
        )
        with torch.no_grad():
