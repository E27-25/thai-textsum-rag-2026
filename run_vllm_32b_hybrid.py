...
TOP_K_RETRIEVE = 30   # wider net for MMR diversification
RERANK_POOL    = 14   # rerank top-14, MMR down to 7
TOP_K_FINAL    = 7    # matches training distribution
MMR_LAMBDA     = 0.7  # relevance weight in MMR (0.0=pure diversity, 1.0=pure relevance)
SPARSE_WEIGHT  = 0.3
BM25_WEIGHT    = 0.2
...
class BGERetriever:
    ...
    def rerank_with_scores(self, query: str, passages: list[dict], top_k: int) -> list[tuple[dict, float]]:
        """Like rerank but returns (passage, relevance_score) — for MMR."""
        queries = [query] * len(passages)
        texts   = [p["text"][:800] for p in passages]
        enc = self.reranker_tok(
            queries, texts, padding=True, truncation=True,
        ...
        if isinstance(scores, float):
            scores = [scores]
        ranked = sorted(range(len(passages)), key=lambda i: scores[i], reverse=True)[:top_k]
        return [(passages[i], scores[i]) for i in ranked]


def mmr_select(scored_pool: list[tuple[dict, float]], dense_index: torch.Tensor,
               id_to_pos: dict, top_k: int, lambda_: float) -> list[dict]:
    """Maximal Marginal Relevance — re-rank scored pool for diversity.
    relevance: cross-encoder score (already in scored_pool)
    similarity: cosine between BGE dense vectors of candidates
    """
    if len(scored_pool) <= top_k:
        return [p for p, _ in scored_pool]

    dense_norm = F.normalize(dense_index, p=2, dim=1)
    rel = {p["para_id"]: s for p, s in scored_pool}

    selected: list[dict] = []
    remaining = [p for p, _ in scored_pool]

    while remaining and len(selected) < top_k:
        if not selected:
            best = max(remaining, key=lambda p: rel[p["para_id"]])
        else:
            sel_vecs = torch.stack([dense_norm[id_to_pos[p["para_id"]]] for p in selected])
            def mmr_score(p: dict) -> float:
                v = dense_norm[id_to_pos[p["para_id"]]]
                max_sim = float((sel_vecs @ v).max().item())
                return lambda_ * rel[p["para_id"]] - (1 - lambda_) * max_sim
...
def main():
    ...
    print("Hybrid retrieve + cross-encoder rerank + MMR for all queries ...", flush=True)
    retrieved_all = []
    for q in tqdm(queries, desc="Retrieve+Rerank+MMR", dynamic_ncols=False):
        paras, index = index_map[q["doc_id"]]
        candidates = retriever.retrieve(q["query"], paras, index, top_k=TOP_K_RETRIEVE)
        scored = retriever.rerank_with_scores(q["query"], candidates, top_k=RERANK_POOL)
        id_to_pos = {p["para_id"]: i for i, p in enumerate(paras)}
        retrieved = mmr_select(scored, index["dense"], id_to_pos,
                               top_k=TOP_K_FINAL, lambda_=MMR_LAMBDA)
