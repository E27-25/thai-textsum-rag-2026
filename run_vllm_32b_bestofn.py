#!/usr/bin/env python3
"""Best-of-N + reranker pick. Generate n=5 candidates, pick best by reranker score."""
...
# Tight params to compensate for n=5 sampling cost
TOP_K_RETRIEVE = 15
TOP_K_FINAL    = 5
SPARSE_WEIGHT  = 0.3
BM25_WEIGHT    = 0.2
...
def main():
    ...
    print(f"Generating {len(prompts)} × n={N_SAMPLES} answers ...", flush=True)
    ...
    for q, retrieved, output in zip(queries, retrieved_all, outputs):
        ...
        # Parse all N candidates, filter valid JSON
        candidates = []
        for o in output.outputs:
            abst, refs = parse_output(o.text, fallback, valid_ids)
            if abst and refs and abst != o.text.strip():  # only valid parsed JSON
                candidates.append((abst, refs))

        if not candidates:
            # All failed parsing → use first one
            abst, refs = parse_output(output.outputs[0].text, fallback, valid_ids)
            candidates = [(abst, refs)]

        # Score each abstractive vs query with reranker, pick best
        if len(candidates) > 1:
            abst_texts = [c[0] for c in candidates]
            scores = _cross_score(q["query"], abst_texts)
            best_idx = int(torch.tensor(scores).argmax())
            abstractive, refs = candidates[best_idx]
        else:
            abstractive, refs = candidates[0]
