...
PODMAN_ROOT="/dev/shm/podman-root7-${USER}"
PODMAN_RUN="/tmp/podman-run7-${USER}"
...
echo "=== [$(date)] Build start (32B DPO AWQ) ==="
...
podman build \
  ...
  --ignorefile .dockerignore.32b_dpo \
  -t textsum-32b-dpo:v1 \
  -f Dockerfile.32b_dpo \
  .
...
for attempt in 1 2 3 4 5; do
  ...
  podman push \
    ...
    --runroot="$PODMAN_RUN" \
    textsum-32b-dpo:v1 \
    'registry.ai.in.th/2026-textsum/48f0b4ab/watin-promfiy.tme5:AI-Benchmark-Programs-2026-32b-dpo-v1' && break
  echo "Push failed, retrying in 60s..."
  sleep 60
done

echo "=== [$(date)] Push done! ==="
echo "Tag: AI-Benchmark-Programs-2026-32b-dpo-v1"
